...
from pr8 import my_callback as callback
...
def gen_data(size=500, img_size=50):
    ...
    label_c1 = numpy.full([c1, 1], 'Horizontal')
    data_c1 = numpy.array([gens.gen_h_line(img_size) for i in range(c1)])
    label_c2 = numpy.full([c2, 1], 'Vertical')
    data_c2 = numpy.array([gens.gen_v_line(img_size) for i in range(c2)])

    data = numpy.vstack((data_c1, data_c2))
    label = numpy.vstack((label_c1, label_c2))
...
model.fit(x_train, y_trn,
          ...
          epochs=epochs,
          validation_data=(x_test, y_tst), callbacks = [callback(x_train, y_trn)])
